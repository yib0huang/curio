...
模型生成期间，输入区将发送按钮切换为停止按钮。用户主动停止时，控制器通过 `AbortController` 中止当前压缩或流式回答请求，`ConversationStore` 冻结已经接收的内容并标记为已停止；主动停止不作为错误提示，也不会恢复已发送的问题草稿。普通请求失败仍回滚未完成轮次并恢复草稿，便于重新发送。
